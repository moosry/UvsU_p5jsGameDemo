...
import { RecordPlayState } from "./RecordPlayState.js";
...
export class RecordSystem {
    constructor(player, maxRecordTime, addReplayerCallback, removeReplayerCallback) {
        ...
        //状态转移规则（使用意图而不是键码）
        this.states = {
            [RecordPlayState.ReadyToRecord]: {
                "record": RecordPlayState.Recording,
            },
            [RecordPlayState.Recording]: {
                "record": RecordPlayState.ReadyToReplay,
                "RecordTimeout": RecordPlayState.ReadyToReplay,
            },
            [RecordPlayState.ReadyToReplay]: {
                "replay": RecordPlayState.Replaying,
                "record": RecordPlayState.Recording,
            },
            [RecordPlayState.Replaying]: {
                "ReplayTimeout": RecordPlayState.ReadyToReplay,
                "replay": RecordPlayState.ReadyToReplay, // 按 R 键提前停止回放
            },
        };
        this.actions = {
            [RecordPlayState.ReadyToRecord]: {
        ...
        //决定状态的属性
        this.state = RecordPlayState.ReadyToRecord;
        this.clip = null;
    ...
    eventHandler(event) {
        ...
        if (intent !== null) {
            ...
            if (intent === "record" && (this.state === RecordPlayState.ReadyToRecord || this.state === RecordPlayState.ReadyToReplay)) {
    ...
    pauseForGamePause() {
        if (this.state === RecordPlayState.Recording) {
        ...
        if (this.state === RecordPlayState.Replaying) {
    ...
    printRecords() {
        ...
        for (const record of records) {
            // Use keyCodeToLabel to print a human-readable key label
            const label = keyCodeToLabel(record["code"]);
            console.log(`eventType: ${record["keyType"]}, key: ${label}`);
        }
    }
