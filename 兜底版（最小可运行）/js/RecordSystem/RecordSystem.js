...
export class RecordSystem {
    ...
    draw() {
        ...
        sketch.rect(0, -60, 960, 60);   // 顶部 UI 背景

        let msg = "";
        let color = sketch.color(255);

        switch (this.state) {
            case "ReadyToRecord":
                msg = "按 E 开始录制";
                color = sketch.color(100, 200, 255);
                break;
            case "Recording":
                msg = "录制中… 按 E 结束";
                color = sketch.color(255, 150, 100);
                break;
            case "ReadyToReplay":
                msg = "按 R 回放，按 E 重新录制";
                color = sketch.color(180, 255, 120);
                break;
            case "Replaying":
                msg = "回放中…";
                color = sketch.color(255, 220, 80);
                break;
        }

        sketch.fill(color);
        sketch.rect(0, -40, 960, 40);   // 状态条

        sketch.fill(0);
        sketch.textSize(24);
        sketch.textAlign(sketch.CENTER, sketch.CENTER);
        sketch.text(msg, 480, -20);     // 顶部文字（正常方向）
