...
export class RecordSystem {
    ...
    draw(p) {
        ...
        // Gray-out overlay when player is airborne and recording is blocked
        if (isAirBlocked) {
            p.fill(20, 15, 40, 120);
            p.rect(panelX + 4, panelY + 4, panelW - 8, panelH - 8);
            p.fill(190, 170, 230, 190);
        ...
        // Red flash when a blocked attempt was just made
        if (airBlockAge < 380) {
            const flashAlpha = (1 - airBlockAge / 380) * 150;
            p.fill(255, 50, 50, flashAlpha);
            p.rect(panelX + 4, panelY + 4, panelW - 8, panelH - 8);
        }
