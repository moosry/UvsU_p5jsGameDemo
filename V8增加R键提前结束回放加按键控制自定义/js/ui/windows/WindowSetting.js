...
export class WindowSetting extends WindowBase {
  ...
  _buildContent() {
    ...
    try {
      // 测试元素
      const testDiv = p.createDiv('🔧 Window Settings');
      testDiv.style('color', '#f0e0ff');
      testDiv.style('font-size', '14px');
      testDiv.style('margin-bottom', '10px');
      testDiv.parent(area);
      console.log('[WindowSetting._buildContent] Added test div');
      ...
      if (this.keyBindingManager) {
        ...
        console.log('[WindowSetting._buildContent] Config keys:', Object.keys(config));
        console.log('[WindowSetting._buildContent] Config values:', Object.values(config));
        console.log('[WindowSetting._buildContent] Full config:', config);
        
        const entries = Object.entries(config);
        console.log(`[WindowSetting._buildContent] Total entries: ${entries.length}`);
        
        for (const [intent, keyCode] of entries) {
        ...
        console.log(`[WindowSetting._buildContent] Completed creating ${entries.length} keybind rows`);
      } else {
  ...
  _refreshLabels() {
    ...
    // 刷新按键绑定相关的标签
    this._keyBindHeader.html(t('win_keybind'));
  ...
  _createKeyBindRow(parent, intent, keyCode) {
    ...
    resetBtn.elt.title = 'Reset to default';
    resetBtn.parent(row);
  ...
  _startListeningForKey(intent, button) {
    ...
    this._tempKeydownHandler = (event) => {
      ...
      if (existingIntent && existingIntent !== intent) {
        alert(`The key ${this._getKeyLabel(newKeyCode)} is already bound to ${this._getIntentLabel(existingIntent)}`);
        this._stopListeningForKey();
        return;
      }
