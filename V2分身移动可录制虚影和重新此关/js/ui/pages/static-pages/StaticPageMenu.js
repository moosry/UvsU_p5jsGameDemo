...
export class StaticPageMenu extends PageBase {
  ...
  enter() {
    ...
    // PLAY 按钮 → 进入开场动画
    const startBtn = new ButtonBase(p, 'PLAY', 0.461 * p.width, 0.8 * p.height, () => {
    ...
    // SETTINGS 按钮
    const settingsBtn = new ButtonBase(p, 'SETTINGS', 0.362 * p.width, 0.822 * p.height, () => {
    ...
    // Credits 按钮
    const creditsBtn = new ButtonBase(p, 'Credits', 0.588 * p.width, 0.822 * p.height, () => {
